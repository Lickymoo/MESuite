...
import com.buoobuoo.mesuite.mechat.dialogue.DialogueUtils;
import com.buoobuoo.mesuite.mecore.MECorePlugin;
import com.buoobuoo.mesuite.meentities.EntityManager;
import com.buoobuoo.mesuite.meentities.impl.RatEntity;
import com.buoobuoo.mesuite.meentities.impl.npc.CaptainYvesNpc;
import com.buoobuoo.mesuite.meentities.impl.npc.HelpfulNpc;
...
import com.buoobuoo.mesuite.meutils.navigation.RouteSingularPlayer;
...
public class ACT1_MQ1 extends QuestLine {
    public ACT1_MQ1(MEQuestsPlugin plugin) {
        super(plugin, "Washed up", "ACT1_MQ1", "Find signs of life");

        //whenever
        whenever("OVERBOARD", player -> {
            player.teleport(new Location(Bukkit.getWorld(MECorePlugin.MAIN_WORLD_NAME), 185.5, 53, 640, -160, 0));
            DialogueUtils.sendDialogueBox(player, CharRepo.UI_PORTRAIT_CAPTAIN_YVES, "Woah there " + player.getName() + "! You almost drowned.");
        });
        ...
        execute(player -> {
            ...
            Location spawnLoc = new Location(MECorePlugin.getMainWorld(), 188.5, 53, 628.5);
        ...
        marker("OFF_SHIP");
        execute(player -> {
            player.teleport(new Location(Bukkit.getWorld(MECorePlugin.MAIN_WORLD_NAME), 195, 51, 282, -180, 0));
        ...
        execute(player -> {
            ...
            RouteSingularPlayer route = new RouteSingularPlayer(
                    new Location(MECorePlugin.getMainWorld(), 194, 53, 261),
                    new Location(MECorePlugin.getMainWorld(), 195, 52, 274)
            );

            //After npc has ran to player
            Location spawnLoc = new Location(MECorePlugin.getMainWorld(), 194, 53, 261);
        ...
        execute(player -> {
            RouteSingularPlayer route = new RouteSingularPlayer(
                    new Location(MECorePlugin.getMainWorld(), 197, 66, 218),
                    new Location(MECorePlugin.getMainWorld(), 197, 66, 168)
            );
        ...
        execute(player -> {
            EntityManager entityManager = plugin.getEntityManager();
            Location spawnLoc = new Location(MECorePlugin.getMainWorld(), 200, 66, 167);
        ...
        execute(player -> {
            RouteSingularPlayer route = new RouteSingularPlayer(
                    new Location(MECorePlugin.getMainWorld(), 189, 66, 155),
                    new Location(MECorePlugin.getMainWorld(), 186, 66, 133),
                    new Location(MECorePlugin.getMainWorld(), 186, 66, 110)
            );
