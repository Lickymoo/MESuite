...
import com.buoobuoo.mesuite.meentities.gamehandler.event.MEEntityDeathEvent;
...
public interface CustomEntity {
    ...
    default CustomEntity instantiateClone(MEEntitiesPlugin plugin){
    ...
    default void onDeath(){
        ...
        for(UUID uuid : getDamagers()){
            ...
            ProfileData profileData = MEEntitiesPlugin.getInstance().getPlayerManager().getProfile(player);
        ...
        */
        ...
        MEEntityDeathEvent event = new MEEntityDeathEvent(this);
    ...
    default void spawnEntity(MEEntitiesPlugin plugin, Location loc){
        setDead(false);
        setIsReady(true);
        Entity ent = asEntity();
    ...
    default void hideEntity(Player player){
        ...
        PacketUtils.sendPacket(removeEntitiesPacket, player);
    }
    ...
    default void showEntity(Player player){
        ...
        PacketUtils.sendPacket(addEntityPacket, player);
        PacketUtils.sendPacket(setEntityDataPacket, player);
    }

    default void updateTick(){
        for(Player player : Bukkit.getOnlinePlayers()){
            if(getShownPlayers().contains(player.getUniqueId()))
                PacketUtils.sendPacket(new ClientboundTeleportEntityPacket(asEntity()), player);
        }
    }
